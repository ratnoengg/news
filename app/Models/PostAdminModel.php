...
class PostAdminModel extends BaseModel
{
    ...
    public function inputValues()
    {
        return [
            ...
            'sub_title' => inputPost('sub_title'),
            'posted_by_alt' => inputPost('posted_by_alt'),
    ...
    public function addPost($postType)
    {
        ...
        $data['user_id'] = inputPost('user_id')?inputPost('user_id'):user()->id;
        //$data['user_id'] = user()->id;
        $data['posted_by_alt'] = inputPost('posted_by_alt')?inputPost('posted_by_alt'):'';
    ...
    public function pushMessage($id)
    {
        ...
        if($post->status):
            ...
            $headings = $post->title;
            $img =  base_url().'/'.$post->image_small;
            $message =characterLimiter($post->content, 150, '...');

            $content = array("en" => "$message");
            $headings = array("en" => "$headings");
            $fields = array(
                'app_id' => trim(getenv('APP_ID')),
            ...
            curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json; charset=utf-8',
                'Authorization: Basic '.trim(getenv('APP_KEY'))));
