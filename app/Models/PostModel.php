...
class PostModel extends BaseModel
{
    ...
    public function buildQuery($langId = null, $fetchContent = false, $joinTags = false, $isPreview = false)
    {
        ...
        if ($fetchContent) {
            ...
            $this->builder->select('posts.id, posts.lang_id, posts.title,posts.sub_title, posts.title_slug, posts.summary, posts.category_id, posts.image_big, posts.image_slider, posts.image_mid, posts.image_small, posts.image_mime, posts.image_storage, posts.slider_order, posts.featured_order, posts.posted_by_alt, posts.post_type, posts.image_url, posts.user_id, posts.pageviews, posts.post_url, posts.updated_at, posts.created_at');
        }
    ...
    //get featured posts Id
    public function getFeaturedPostsId()
    {
        $this->buildQuery();
        $this->builder->select('posts.id');
        ...
        $rows = $this->builder->get(20)->getResult();
        $results = array();
        foreach ($rows as $row) {
            $results[] = $row->id;
        }
    ...
    {
        $this->buildQuery();
        $this->builder->whereNotIn('posts.category_id', array(39,40,41));
        $this->builder->where('posts.is_featured', 1);
        if ($this->generalSettings->sort_featured_posts == 'by_featured_order') {
            $this->builder->orderBy('posts.featured_order');
        } else {
    ...
    public function getFeaturedEngPosts()
    {
        $this->buildQuery();
        $this->builder->where('posts.is_featured', 1);
       
        $this->builder->whereIn('posts.category_id', array(39,40,41));
        if ($this->generalSettings->sort_featured_posts == 'by_featured_order') {
            $this->builder->orderBy('posts.featured_order');
        } else {
            $this->builder->orderBy('posts.created_at DESC');
    ...
    public function getBreakingNews()
    {
     ...
     //get english_24×X news
    public function getEnglishNews()
    {
        $this->buildQuery();
        return $this->builder->whereIn('posts.category_id', array(39,40,41))->orderBy('posts.created_at DESC')->get($this->breakingPostsLimit)->getResult();
    }
    ...
    {
        $this->buildQuery();
        return $this->builder->whereNotIn('posts.category_id ', array(86))->where('posts.is_recommended', 1)->orderBy('posts.created_at DESC')->get($this->recommendedPostsLimit)->getResult();
    }

    //get recommended posts
    public function getRecommendedAddPosts()
    {
        $this->buildQuery();
        return $this->builder->whereIn('posts.category_id ', array(86))->where('posts.is_recommended', 1)->orderBy('posts.created_at DESC')->get($this->recommendedPostsLimit)->getResult();
    }



     //get Custom posts
    public function getCustomPosts($category_id)
    {
        $category_id = explode(',',$category_id);

        $this->buildQuery();
        return $this->builder->whereIn('posts.category_id', $category_id)->orderBy('posts.created_at DESC')->get($this->recommendedPostsLimit)->getResult();
    }
    ...
    public function getSearchPostsCount($q, $searchInContent)
    {
        ...
        $this->buildQuery();
        $this->builder->groupStart()->like('posts.title', $q)->orLike('posts.summary', $q)->orLike('posts.posted_by_alt', $q);
        if ($searchInContent) {
