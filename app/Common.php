...
if (!function_exists('generateSearchURL')) {
    function generateSearchURL($userSlug)
    {
        if (!empty($userSlug)) {
            return langBaseUrl(getRoute('search?q=', true) . $userSlug);
        }
        return "#";
    }
}
...
//date format with month
if (!function_exists('formatDateTimeFront')) {
    function formatDateTimeFront($timestamp)
    {
        if (!empty($timestamp)) {
            $date = date("M j, Y  H:i", strtotime($timestamp));
            return replaceMonthName($date);
        }
    }
}
...
if (!function_exists('convertToBanglaDate')) {
    ...
    $numbers_bangla = ['০','১','২','৩','৪','৫','৬','৭','৮','৯'];

    $months_english= ["জানুয়ারি", "ফেব্রুয়ারি", "মার্চ", "এপ্রিল", "মে", "জুন", "জুলাই", "আগষ্ট", "সেপ্টেম্বর", "অক্টোবর", "নভেম্বর", "ডিসেম্বর"];
    ...
    for ($j=0; $j< strlen($day); $j++) {
        for ($i=0; $i<=9; $i++) {
            if($day[$j] == $i){
                $day_bangla .=$numbers_bangla[$i];
            }
        }
        
     }

    return $days_bangla[date('w')].', '.$day_bangla.' '.$months_english[date('n')-1].' '.$year_bangla;
...
//get cnagla date 
if (!function_exists('getEngToBanDate')) {
    function getEngToBanDate($timestamp) 
    {
    ...
    $months = date("n", strtotime($timestamp));

    return $day_bangla.' '.$months_english[$months-1].', '.$year_bangla;
...
//get cnagla date 
if (!function_exists('getEngToBanDateTime')) {
    function getEngToBanDateTime($timestamp) 
    {
    ...
    $hour_bangla='';
    for ($j=0; $j< strlen($hour); $j++) {
        for ($i=0; $i<=9; $i++) {
            if($hour[$j] == $i){
                $hour_bangla .=$numbers_bangla[$i];
            }
        }
    ...
    $minutes_bangla='';
    for ($j=0; $j< strlen($minutes); $j++) {
        for ($i=0; $i<=9; $i++) {
            if($minutes[$j] == $i){
                $minutes_bangla .=$numbers_bangla[$i];
            }
        }
    ...
    return $day_bangla.' '.$months_english[$months-1].', '.$year_bangla.' '.$hour_bangla.':'.$minutes_bangla;
...
//get featured posts
if (!function_exists('getFeaturedPostsId')) {
    function getFeaturedPostsId()
    {
        //$featuredPostsId = array();
        $featuredPosts = getCachedData('featured_posts_id');
        if (empty($featuredPosts)) {
            $model = new \App\Models\PostModel();
            $featuredPostsId = $model->getFeaturedPostsId();
            setCacheData('featured_posts_id', $featuredPostsId);
...
//get featured English posts
if (!function_exists('getFeaturedEngPosts')) {
    function getFeaturedEngPosts()
    {
        $featuredPosts = getCachedData('featured_eng_posts');
        if (empty($featuredPosts)) {
            $model = new \App\Models\PostModel();
            $featuredPosts = $model->getFeaturedEngPosts();
            setCacheData('featured_eng_posts', $featuredPosts);
        }
        return $featuredPosts;
    }
}
...
//get english category based  news
if (!function_exists('getEnglishNews')) {
    function getEnglishNews()
    {
        $englishNews = getCachedData('english_24×X');
        if (empty($englishNews)) {
            $model = new \App\Models\PostModel();
            $englishNews = $model->getEnglishNews();
            setCacheData('english_24×X', $englishNews);
        }
...
//get recommended Add posts
if (!function_exists('getRecommendedAddPosts')) {
    function getRecommendedAddPosts()
    {
        $recommendedPosts = getCachedData('recommended_add_posts');
        if (empty($recommendedPosts)) {
            $model = new  \App\Models\PostModel();
            $recommendedPosts = $model->getRecommendedAddPosts();
            setCacheData('recommended_add_posts', $recommendedPosts);
        }
        return $recommendedPosts;
    }
}
...
//get Custom posts
if (!function_exists('getCustomPosts')) {
    function getCustomPosts($category_id)
    { 
        $customPosts = getCachedData('custom_posts'.$category_id);
        if (empty($customPosts)) {
            $model = new  \App\Models\PostModel();
            $customPosts = $model->getCustomPosts($category_id);
            setCacheData('custom_posts'.$category_id, $customPosts);
        }
        return $customPosts;
    }
}
...
if (!function_exists('setPostMetaTags')) {
    function setPostMetaTags($post, $postTags, $data)
    {
        ...
        $data['ogWidth'] = '750';
        $data['ogHeight'] = '422';
        $data['ogCreator'] = $post->posted_by_alt? $post->posted_by_alt : $post->author_username;
        $data['ogAuthor'] = $post->posted_by_alt? $post->posted_by_alt : $post->author_username;
...
if (!function_exists('isBot')) {
    ...
    if (!function_exists('BanglaDate')) {
        function BanglaDate( $timestamp, $hour = 6 ) {
            Globals::$engDate = date( 'd', $timestamp );
            Globals::$engMonth = date( 'm', $timestamp );
            Globals::$engYear = date( 'Y', $timestamp );
            Globals::$morning = $hour;
            Globals::$engHour = date( 'G', $timestamp );
    ...
    if (!function_exists('calculate_date')) { 
         function calculate_date() {     
            //echo Globals::$bn_month_middate[Globals::$engMonth - 1];
            Globals::$bangDate = Globals::$engDate - Globals::$bn_month_middate[Globals::$engMonth - 1];
            if (Globals::$engHour < Globals::$morning) 
                Globals::$bangDate -= 1;
            
            if ((Globals::$engDate <= Globals::$bn_month_middate[Globals::$engMonth - 1]) || (Globals::$engDate == Globals::$bn_month_middate[Globals::$engMonth - 1] + 1 && Globals::$engHour < Globals::$morning) ) {
                Globals::$bangDate += Globals::$bn_month_dates[Globals::$engMonth - 1];
                if (is_leapyear() && Globals::$lipyearindex == Globals::$engMonth) 
                    Globals::$bangDate += 1;
                Globals::$bangMonth = Globals::$bn_months[Globals::$engMonth - 1];
            }
            else{
                Globals::$bangMonth = Globals::$bn_months[(Globals::$engMonth)%12];      
            }
        }
    }
     ...
     */
    if (!function_exists('is_leapyear')) { 
        function is_leapyear() {
            if ( Globals::$engYear % 400 == 0 || (Globals::$engYear % 100 != 0 && Globals::$engYear % 4 == 0) )
                return true;
            else
                return false;
        }
    ...
    if (!function_exists('calculate_year')) { 
        function calculate_year() {
            Globals::$bangYear = Globals::$engYear - 593;
            if ((Globals::$engMonth < 4) || ((Globals::$engMonth == 4) && ((Globals::$engDate < 14) || (Globals::$engDate == 14 && Globals::$engHour < Globals::$morning))))
                Globals::$bangYear -= 1;
        }
    }
    ...
    if (!function_exists('convert')) { 
        function convert() {
            Globals::$bangDate = bangla_number( Globals::$bangDate );
            Globals::$bangYear = bangla_number( Globals::$bangYear );
        }
    }
    ...
    if (!function_exists('get_date')) { 
        function get_date() {
            return ', '.Globals::$bangDate.' '.Globals::$bangMonth.' '.Globals::$bangYear.' বঙ্গাব্দ';
        }
    }
